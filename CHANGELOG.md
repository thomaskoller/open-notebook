...
### Changed
- **Podcast generation runs in this repository instead of the `podcast-creator` library** ([ADR-010](docs/7-DEVELOPMENT/decisions/ADR-010-own-podcast-pipeline.md)). The new `open_notebook/podcasts/pipeline.py` does outline → per-segment transcript → batched TTS → ffmpeg concat in three separately callable stages, so the outline and transcript are persisted to the episode **before** audio synthesis starts (a TTS or ffmpeg failure no longer discards the LLM work you already paid for) and the jobs drawer narrates real stages with counts (`generating_outline`, `generating_transcript n/m`, `generating_audio n/m`, `combining_audio`) instead of one `generating_audio` message for the whole run. The command lost the ~80 lines that existed only to rebuild and prune the library's global all-profiles config on every generation, so an unrelated broken or orphaned profile can no longer affect an episode, and `num_segments` above 10 works (the library capped it at 10 while the profile allows 3–20). Audio is combined with ffmpeg (already present in the Docker images) rather than moviepy, which drops `moviepy`, `pydub`, `imageio-ffmpeg` and `proglog` from the dependency tree and with them the `pillow>=12.2.0` override that worked around moviepy's `pillow<12` cap — Pillow now resolves to 12.3.0 on its own. The dormant `configure("templates", ...)` SSTI footgun documented in `security.md` is gone with the dependency. Episode/speaker profiles, the stored `outline`/`transcript` shapes, the API and the no-auto-retry policy are unchanged
...
### Fixed
- **The jobs indicator no longer counts work that will never finish.** Every terminal job transition is written by a Celery signal inside the worker, so a worker killed mid-task (container restart, OOM) left its row `running` and a queued message lost with the broker left it `queued` — forever. The sidebar could report "25 in progress" with nothing running, and a source whose processing worker died showed "processing" indefinitely. A queued/running/retrying row that has had no update for `OPEN_NOTEBOOK_JOB_STALE_MINUTES` (default 30) is now reported as `unknown`, which drops it from the in-progress count and stops the 2-second poll. This is computed when the status is read, not written to the database, so a job that merely went quiet between progress reports shows as active again the moment it reports anything
- **Long prompts are usable again in the podcast dialog.** The generate-episode dialog was `overflow-hidden` with no inner scroll container — the only dialog in the app like that — so as the auto-growing instructions field expanded it grew past the bottom of the dialog and was simply clipped, with no way to scroll to what you were typing. The dialog now scrolls (the pattern the other dialogs already use), and every `Textarea` in the app stops growing at 50vh and scrolls internally, so a long prompt can no longer push a form's own buttons out of reach. The inline (notebook name/description) editor also grows with its content now instead of staying two lines tall
- **A single mistyped key in the model's JSON no longer kills a whole podcast episode.** An episode failed with `Failed to parse ValidatedTranscript ... transcript.2.dialogue Field required` because the transcript model wrote `"dialogle"` in one dialogue item out of eight. The retry that should have re-sampled that segment never fired: it classified "permanent" failures with a denylist containing `ValueError`, and LangChain's `OutputParserException` subclasses `ValueError`, so the outline and every segment already generated were thrown away on the first attempt. Parse failures (`OutputParserException`, pydantic `ValidationError`, `JSONDecodeError`) are now checked first by an explicit allowlist and always retried, while genuine configuration errors still fail immediately; tune with `PODCAST_RETRY_MAX_ATTEMPTS` / `PODCAST_RETRY_WAIT_MULTIPLIER`. A hallucinated speaker name is likewise retried instead of surfacing later as a voice-lookup failure, and an exhausted retry now reports which model to change instead of dumping the raw LangChain error into the UI
- **Podcast language settings apply again.** The repo's copies of `prompts/podcast/*.jinja` shadow the ones the library shipped but had lost their `{% if language %}` block, so `EpisodeProfile.language` had no effect on generated content — a `pt-BR` profile still produced an English episode. Both templates carry the language instruction again, with the code resolved to a language name (`pt-BR` → `Portuguese`)
- **Podcast audio failures are reported as failures.** Audio combination used to hand back an `"ERROR: ..."` string inside the result dict, which the caller had to string-match; the pipeline raises instead, with ffmpeg's own stderr attached
- API password authentication now accepts UTF-8 non-ASCII passwords; clients sending non-ASCII passwords as Latin-1 bytes must switch to UTF-8 (#1290)
